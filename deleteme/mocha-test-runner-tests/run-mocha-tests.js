...
test('Mocha test run fails when standard ui run against all-bad-standard-ui', function(done){
	var	mockTestMonitor = {
		...
		testOptions = {
			tests : ['dummy-tests/all-bad-standard-ui.js']
		};
		mochaTestRunner = new MochaTestRunner(testOptions, mockTestMonitor);
	mochaTestRunner.start();
});

test('2 Mocha test run fails when standard ui run against all-okay-standard-ui & all-bad-standard-ui', function(done){
	var	mockTestMonitor = {
			passed : function(){
				done('Expected test runner failure');
			},
		...
		testOptions = {
			tests : [
				'dummy-tests/all-bad-standard-ui.js',
				'dummy-tests/all-okay-standard-ui.js'
			]
		};
		mochaTestRunner = new MochaTestRunner(testOptions, mockTestMonitor);
	mochaTestRunner.start();
